...
Class Gregphoto_Image {
	...
	protected function getThumbnailParams($type) {
		$w = $this->getWidth();
		$h = $this->getHeight();
		$nw = null;
		$nh = null;
		...
		$y = 0;
		switch($type) {
			case Gregphoto_Image::CROP_FIT:
				if(($w / $h) > ($this->maxWidth / $this->maxHeight)) {
					$nh = $this->maxHeight;
					$nw = intval(($nh * $w) / $h);
					$x = round(($nw - $this->maxWidth) / 2);
				} else {
					$nw = $this->maxWidth;
					$nh = intval(($nw * $h) / $w);
	...
	protected function crop($img, $x_offset, $y_offset) {
		...
		imagecopy(
			...
			$img,
			0, 0, // dst xy
			$x_offset, $y_offset, // src xy
			imagesx($img), imagesy($img) // src dim
		);
		return $this->croped_thumb;
	}
	...
	public function __destruct() {
		...
		if(is_resource($this->croped_thumb)) imagedestroy($this->croped_thumb);
	}
}
